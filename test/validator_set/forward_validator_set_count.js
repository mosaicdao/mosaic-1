...
async function assertForwardValidatorSetCount(validatorSet, height, count) {
  const actualCount = await validatorSet.forwardValidatorSetCount(height);
  assert.isOk(
    actualCount.eq(count),
  );
}

contract('ValidatorSet::forwardValidatorSetCount', (accounts) => {
  beforeEach(async () => {
  ...
  contract('Positive Tests', () => {
    ...
    it('checks validator set count', async () => {
      ...
      await assertForwardValidatorSetCount(config.validatorSet, height0, new BN(1));
      ...
      await assertForwardValidatorSetCount(config.validatorSet, height0, new BN(1));
      await assertForwardValidatorSetCount(config.validatorSet, height1, new BN(3));
      ...
      await assertForwardValidatorSetCount(config.validatorSet, height0, new BN(1));
      await assertForwardValidatorSetCount(config.validatorSet, height1, new BN(3));
      await assertForwardValidatorSetCount(config.validatorSet, height2, new BN(2));
      ...
      await assertForwardValidatorSetCount(config.validatorSet, height1, new BN(3));
      await assertForwardValidatorSetCount(config.validatorSet, height2, new BN(2));
      await assertForwardValidatorSetCount(config.validatorSet, height3, new BN(2));
      ...
      await assertForwardValidatorSetCount(config.validatorSet, height2, new BN(2));
      await assertForwardValidatorSetCount(config.validatorSet, height3, new BN(2));
      await assertForwardValidatorSetCount(config.validatorSet, height4, new BN(0));
      ...
      await assertForwardValidatorSetCount(config.validatorSet, height3, new BN(2));
      await assertForwardValidatorSetCount(config.validatorSet, height4, new BN(0));
      await assertForwardValidatorSetCount(config.validatorSet, height5, new BN(0));
    });
  });
});
